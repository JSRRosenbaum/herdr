...
The default is `show_workspace_ids = false`; with it off the sidebar layout and hit targets match the upstream layout exactly. The tags render in a fixed-width column sized to the longest workspace ID in the current view, so rows stay aligned across workspaces. Changes apply through the usual `reload config` action.
