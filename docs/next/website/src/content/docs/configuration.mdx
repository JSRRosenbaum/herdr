...
Worktrees render in the expanded sidebar with the two-line `tree` layout by default: parents sit flush and children indent with deep `   ├─`/`   └─` connectors. Set `worktree_layout = "compact"` to compress the presentation:
...
Compact mode shallows the child connectors to `├─`/`└─` and, when you have not set `rows` explicitly, collapses the Space rows to a single line (`state_icon`, `workspace`, `branch`, `git_status`), separating the workspace from its branch with ` - ` instead of ` · `. An explicit `rows` always wins and keeps the ` · ` separator. The default is `worktree_layout = "tree"`, which matches the upstream layout exactly.
### Workspace ID tags

Set `show_workspace_ids = true` to right-align the stable workspace ID (for example `wAB`) on each Space row, after the group toggle:
...
The default is `show_workspace_ids = false`; with it off the sidebar layout and hit targets match the upstream layout exactly. The tags render in a fixed-width column sized to the longest workspace ID in the current view, so rows stay aligned across workspaces.
